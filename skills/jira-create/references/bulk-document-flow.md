...
**여기서도 깊게 파지 않는다.** 후속 grill 문답(harness-workflow ②)·`/jira-plan` 단계의 몫.
...
mcp__atlassian__searchJiraIssuesUsingJql
  jql: 'project = "STD" AND issuetype = Epic AND statusCategory != Done AND (summary ~ "<키워드1>" OR summary ~ "<키워드2>")'
...
이대로 등록할까요?
- [수정사항 있으면 알려주세요 — 트리/제목/라벨 어느 것도 가능]
```
...
다음 단계: 작업할 이슈를 골라 /harness-workflow <KEY>
```
