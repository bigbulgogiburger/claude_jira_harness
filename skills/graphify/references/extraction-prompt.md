# Step 3B — Semantic extraction: the subagent prompt

**When to read this file:** You are dispatching subagents in Step 3 Part B (semantic extraction). Read this to get the exact prompt to send each subagent.
...
After all subagents return, continue with SKILL.md Step 3B3 (collect, cache, merge).

## Why a discrete rubric instead of "0.0-1.0 range"

Models follow discrete rubrics better than continuous ranges. Production data showed a bimodal distribution (>50% at 0.5, >40% at 0.85+) — meaning the range guidance was being collapsed to a binary. Forcing one of five named values produces calibrated confidence_scores.
